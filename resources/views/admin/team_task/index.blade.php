        ...
        {!! Form::open(['method' => 'POST', 'action' => 'AdminTeamTaskController@store']) !!}
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group{{ $errors->has('team_id') ? ' has-error' : '' }}">
                  ...
                  {!! Form::select('team_id', array(''=>'Choose Member') + $teams, null, ['class' => 'form-control select2 myform', 'required' => 'required']) !!}
                  <small class="text-danger">{{ $errors->first('team_id') }}</small>
                </div>
                <div class="form-group{{ $errors->has('user_id') ? ' has-error' : '' }}">
                  {!! Form::label('user_id', 'Customer') !!}
                  {!! Form::select('user_id', array(''=>'Choose Customer') + $users, null, ['class' => 'form-control select2 myform', 'required' => 'required']) !!}
                  <small class="text-danger">{{ $errors->first('user_id') }}</small>
                </div>
                <div class="form-group{{ $errors->has('status_id') ? ' has-error' : '' }}">
                  {!! Form::label('status_id', 'Status') !!}
                  {!! Form::select('status_id', array('0'=>'Choose Status') + $statuses, null, ['class' => 'form-control select2 myform']) !!}
              ...
              <div class="col-md-6">
                <div class="form-group{{ $errors->has('task') ? ' has-error' : '' }}">
                  ...
                  {!! Form::textarea('task', null, ['class' => 'form-control myform', 'rows'=>'9', 'required' => 'required', 'placeholder'=>'Create Task']) !!}
          ...
          <div class="modal-footer">
            <div class="btn-group pull-right">
             <!--  {!! Form::reset("Reset", ['class' => 'btn btn-yellow btn-default myform' ]) !!} -->
        ...
        <tbody>
          ...
          @foreach ($tasks as $task)
            <tr>
             ...
             word-break: break-all;">{{$task->task}}</td>
              <td>
                @if ($task->status_id == '0')
                  None
                @else
