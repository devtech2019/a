...
@section('content')
    ...
    <form method="POST" class="card card-primary" action="{{WEBSITE_ADMIN_NAV_URL}}{{$selectedNavigation->url . (isset($item)? "/{$item->id}" : '')}}" accept-charset="UTF-8">
        {!! csrf_field() !!}
    ...
    @if(isset($item))
        @include('admin.pages.components.components', ['page' => $item, 'url' => "sections"])
    @endif
@endsection
