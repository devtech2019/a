 ...
 <div class="card <!--card-outline--> card-primary">
        <div class="card-header">
            <h3 class="card-title">
                ...
                <span>{{ isset($item)? 'Edit the ' . $item['name'] . ' entry': 'Create a new Email Template ' }}</span>
            </h3>
        ...
        <form method="POST" 
        action='{{isset($item)?route("EmailLogs.update",array($item["id"])):route("EmailLogs.save")}}' accept-charset="UTF-8" enctype="multipart/form-data">
        {!! csrf_field() !!}
    ...
    <script type="text/javascript" >
        ...
        function appendConstants(){
            ...
            if(value){
                $.ajax({
                    type    :   "POST",
                    url     :   "{{route('EmailLogs.getConstant')}}",
                    data    :   {"action" : value},
