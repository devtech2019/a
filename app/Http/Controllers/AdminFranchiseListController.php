...
class AdminFranchiseListController extends Controller
{
    ...
    public function listlogs(Request $request){
           ...
           if ($request->isMethod('post')) {
            ...
            $totalRecords           =  FranchiseRequest::select('count(*) as allcount')->count();
            $totalRecordswithFilter =   FranchiseRequest::select('count(*) as allcount')->where($configDataTable['conditions'])->count();
            $records                =   FranchiseRequest::orderBy($columnName, $columnOrderType)->where($configDataTable['conditions'])
            ...
            if($response){
               ...
               $adminListUrl   =   route('franchiselist.index');
              return view('admin.franchiselist.index',compact('adminListUrl'));
  ...
  /**
    ...
    public function updateStatus($Id = 0,$modelStatus = 0){
        ...
        if($franchiseData->status == 1){        
            ...
            if(isset( $franchiseData['email']) && !empty( $franchiseData['email'])){
                $to              =    $franchiseData['email']??'';
                $to_name         =    $franchiseData['name']??'';
                $admin_name      =    ' ';
                $full_name       =    $franchiseData['name']??'';
                $email           =    $franchiseData['email']??'';
                $contact_no      =    $franchiseData['contact_no']??'';
                $company_name    =    $franchiseData['company_name']??'';
                $address         =    $franchiseData['address']??'';
                $state           =    $franchiseData['state']??'';
                $city            =    $franchiseData['city']??'';
                $pincode         =    $franchiseData['pincode']??'';
                $action          =     "reject_franchise_registration"; 
            ...
            if(isset( $franchiseData['email']) && !empty( $franchiseData['email'])){
                ...
                $city            =    $franchiseData['city']??'';
                $pincode         =    $franchiseData['pincode']??'';
                $action          =     "approve_franchise_registration"; 
                $rep_Array       =     array($admin_name,$email, $contact_no,$company_name,$address,$state,$city,$pincode); 
